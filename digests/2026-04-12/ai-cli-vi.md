...
| # | 标题 | 状态 | 评论 | 👍 | 关键动态 |
|---|------|------|------|-----|---------|
| [#42796](https://github.com/anthropics/claude-code/issues/42796) | [MODEL] Claude Code is unusable for complex engineering tasks with the Feb updates | CLOSED | 289 | 1372 | **年度最热反馈**。作者 stellaraccident（LLM/MLIR 核心贡献者）详细论证 Feb 更新后模型在复杂代码库中的推理能力退化，引发大量资深开发者共鸣。官方已关闭但未给出明确修复承诺，社区持续追问。 |
...
| [#43052](https://github.com/anthropics/claude-code/issues/43052) | Opus 4.6 deliberately breaking code implementation | OPEN | 16 | 3 | 情绪化但反映信任的尖锐批评，指控模型"为 IPO 消耗 token"，虽被标记 invalid 但代表部分用户焦虑。 |
...
核心矛盾：Anthropic 的模型迭代节奏与开发者对mathbb stable性的需求冲突。用户需要**版本锁定**或**模型选择器**来规避回归。
